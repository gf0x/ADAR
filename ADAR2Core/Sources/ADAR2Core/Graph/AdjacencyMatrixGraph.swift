//
//  AdjacencyMatrixGraph.swift
//  Graph
//
//  Created by Andrew McKnight on 5/13/16.
//

import Foundation

open class AdjacencyMatrixGraph<T, U>: AbstractGraph<T, U> where T: Hashable, U: Hashable & OptionSet {

    fileprivate struct Adjacency {
        let weight: Double?
        let userInfo: U?
    }

    // If adjacencyMatrix[i][j] is not nil, then there is an edge from
    // vertex i to vertex j.
    fileprivate var adjacencyMatrix: [[Adjacency?]] = []
    fileprivate var _vertices: [Vertex<T>] = []
    
    public required init() {
        super.init()
    }
    
    public required init(fromGraph graph: AbstractGraph<T, U>) {
        super.init(fromGraph: graph)
    }
    
    open override var vertices: [Vertex<T>] {
        return _vertices
    }
    
    open override var edges: [Edge<T, U>] {
        var edges = [Edge<T, U>]()
        for row in 0 ..< adjacencyMatrix.count {
            for column in 0 ..< adjacencyMatrix.count {
                if let adjacency = adjacencyMatrix[row][column] {
                    edges.append(Edge(
                        from: vertices[row], to: vertices[column],
                        weight: adjacency.weight, userInfo: adjacency.userInfo
                    ))
                }
            }
        }
        return edges
    }
    
    // Adds a new vertex to the matrix.
    // Performance: possibly O(n^2) because of the resizing of the matrix.
    open override func createVertex(_ data: T) -> Vertex<T> {
        // check if the vertex already exists
        let matchingVertices = vertices.filter { vertex in
            return vertex.data == data
        }
        
        if matchingVertices.count > 0 {
            return matchingVertices.last!
        }
        
        // if the vertex doesn't exist, create a new one
        let vertex = Vertex(data: data, index: adjacencyMatrix.count)
        
        // Expand each existing row to the right one column.
        for i in 0 ..< adjacencyMatrix.count {
            adjacencyMatrix[i].append(nil)
        }
        
        // Add one new row at the bottom.
        let newRow = [Adjacency?](repeating: nil, count: adjacencyMatrix.count + 1)
        adjacencyMatrix.append(newRow)
        
        _vertices.append(vertex)
        
        return vertex
    }
    
    open override func addDirectedEdge(_ from: Vertex<T>, to: Vertex<T>, withWeight weight: Double?, userInfo: U?) {
        adjacencyMatrix[from.index][to.index] = Adjacency(weight: weight, userInfo: userInfo)
    }
    
    open override func addUndirectedEdge(_ vertices: (Vertex<T>, Vertex<T>), withWeight weight: Double?, userInfo: U?) {
        addDirectedEdge(vertices.0, to: vertices.1, withWeight: weight, userInfo: userInfo)
        addDirectedEdge(vertices.1, to: vertices.0, withWeight: weight, userInfo: userInfo)
    }
    
    open override func weightFrom(_ sourceVertex: Vertex<T>, to destinationVertex: Vertex<T>) -> Double? {
        return adjacencyMatrix[sourceVertex.index][destinationVertex.index]?.weight
    }
    
    open override func edgesFrom(_ sourceVertex: Vertex<T>) -> [Edge<T, U>] {
        var outEdges = [Edge<T, U>]()
        let fromIndex = sourceVertex.index
        for column in 0..<adjacencyMatrix.count {
            if let adjacency = adjacencyMatrix[fromIndex][column] {
                outEdges.append(Edge(
                    from: sourceVertex, to: vertices[column], 
                    weight: adjacency.weight, userInfo: adjacency.userInfo
                ))
            }
        }
        return outEdges
    }
    
    open override var description: String {
        var grid = [String]()
        let n = self.adjacencyMatrix.count
        for i in 0..<n {
            var row = ""
            for j in 0..<n {
                if let value = self.adjacencyMatrix[i][j]?.weight {
                    let number = NSString(format: "%.1f", value)
                    row += "\(value >= 0 ? " " : "")\(number) "
                } else {
                    row += "  ø  "
                }
            }
            grid.append(row)
        }
        return (grid as NSArray).componentsJoined(by: "\n")
    }
    
}
