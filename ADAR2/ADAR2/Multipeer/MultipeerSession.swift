...
protocol MultipeerSessionDelegate: AnyObject {
    ...
    /// Return `true` to accept the discovered peer.
...
class MultipeerSession: NSObject {
    ...
    static let serviceType = "adar-collab"
...
extension MultipeerSession: MCSessionDelegate {
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        switch state {
        case .connected: delegate?.multipeerSession(self, peerDidConnect: peerID)
        case .notConnected: delegate?.multipeerSession(self, peerDidDisconnect: peerID)
        default: break
        }
    }

    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        delegate?.multipeerSession(self, didReceive: data, from: peerID)
    }
...
extension MultipeerSession: MCNearbyServiceBrowserDelegate {
    ...
    func browser(_ browser: MCNearbyServiceBrowser, foundPeer peerID: MCPeerID, withDiscoveryInfo info: [String: String]?) {
        let accepted = delegate?.multipeerSession(self, shouldAcceptPeer: peerID) ?? false
        if accepted { browser.invitePeer(peerID, to: session, withContext: nil, timeout: 10) }
    }
